    ...
    return (
        <Link to={`/item/${product.id}`} className='max-w-[300px] max-h-[450px] flex flex-col justify-between w-full hover:shadow-2xl duration-500 transition-all h-full p-4 bg-white relative rounded'>
            <div className='w-full h-full max-w-[270px] max-h-[270px] overflow-hidden rounded-md'>
                <img className='w-full object-cover h-full hover:scale-125 duration-1000 transition-all cursor-pointer' src={product.images[0]} alt={product.name} />
            </div>
                ...
                <div className=''>
                    {product.discount ? (
                        <div className='flex justify-between'>
                            <span className="font-semibold line-through opacity-70 text-lg">${product.price}</span>
                            <span className="font-bold text-red-600 text-lg">${product.discountedPrice}
                        </span>
                        </div>
                    ) : (
