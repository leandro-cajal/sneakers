...
export const Checkout = () => {
  ...
  const handleSubmit = async (event) => {
    if (formData.fullName && formData.email && formData.address && formData.phone && selectedOption !== "") {
      ...
      toast.error("Debe completar los campos del formulario y forma de pago para finalizar la compra", {
        position: "top-center",
        autoClose: 5000,
  ...
  const handleApplyDiscount = (event) => {
    ...
    event.preventDefault();
    setDiscountApply(discountCode === validDiscountCode);
  };
  ...
  useEffect(() => {
    ...
    discountApply ? setFinalPrice(prevFinalPrice => parseFloat((prevFinalPrice - discountAmount).toFixed(2))) : "";
  }, [discountApply]);
                      ...
                      {cart.length > 0 && cart.map((prod) => (
                        <li className={`py-4 flex gap-4 text-sm text-gray-400 ${cart.length > 1 && 'border-b'}`} key={prod.id}>
                          ...
                          <div>
                            ...
                            <span className='font-semibold'>$ {prod.discount ? prod.discountedPrice : prod.price}.00 x {prod.stock}</span>
                          </div>
                        </li>
                      ))}
                    </ul>
